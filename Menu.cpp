...
{	
	...
	{	
		if(i->state == STANDARD)
			gGraphics->drawTexture(i->standardTexture, i->itemRect, 0xFFFFFFFF, 0);
		else if(i->state = SELECTED)
			gGraphics->drawTexture(i->onSelectTexture, i->itemRect, 0xFFFFFFFF, 0);
...
void Menu::update(Vector mousePos)
{
	if(mNavigation == MOUSE)
	{
		...
		while( i != mMenuItemList.end())
		{
			...
			{
				i->state = SELECTED;
				if(gDInput->mouseButtonPressed(0) && i->pressable)	{
